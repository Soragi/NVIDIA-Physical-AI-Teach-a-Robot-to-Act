...
#
# Usage: workshop/scripts/deploy_vss_base.sh check|deploy|refresh-ui|status|stop
...
STATE_ROOT="${XDG_STATE_HOME:-${HOME}/.local/state}/vss-robotics-workshop"
...
# Docker CE to the newest supported package before starting VSS.
...
check_docker() {
  ...
  if ! version_at_least "$docker_version" "$MIN_DOCKER_VERSION" || ! version_before "$docker_version" "$MAX_DOCKER_VERSION_EXCLUSIVE"; then
    ...
    note "On this clean Brev VM, the deploy step will pin Docker CE to the newest compatible release and restart Docker."
    return 0
  fi
...
pin_docker_to_supported_version() {
  ...
  [[ -n "$package_version" ]] || die "No compatible Docker CE package was found. Recreate the Brev VM with a VSS-compatible image or install Docker CE ${MIN_DOCKER_VERSION} within the supported range from Docker's apt repository."
...
configure_docker_storage_if_needed() {
  ...
  # A prepared robotics clone also contains GR00T environments and checkpoints.
  # The initial-download allowance is not required again for cached VSS images.
  if (( docker_free >= 80 )) && docker image inspect \
      nvcr.io/nim/nvidia/cosmos3-reasoner:1.7 \
      nvcr.io/nim/nvidia/nvidia-nemotron-nano-9b-v2:1 \
      nvcr.io/nvidia/vss-core/vss-agent:3.2.1 >/dev/null 2>&1; then
    note "Using cached VSS model images with ${docker_free} GiB working headroom."
    return 0
  fi
...
VST_INTERNAL_URL=http://127.0.0.1:30888
...
prepare_workshop_data() {
  ...
  if ! mkdir -p \
    ...
    "${data_root}/data_log/vst/vst_data/logs/nginx_logs"; then
    sudo mkdir -p \
      "${data_root}/data_log/redis/data" \
      "${data_root}/data_log/redis/log" \
      "${data_root}/data_log/vst/vst_data" \
      ...
      "${data_root}/data_log/vst/temp_files" \
      "${data_root}/data_log/vst/clip_storage" \
    "${data_root}/data_log/vst/vst_data/logs/nginx_logs"
  fi
...
wait_for_service() {
  ...
  while (( elapsed < timeout_seconds )); do
    ...
    if [[ "$status" == unhealthy || "$status" == exited || "$status" == dead ]]; then
      note "Service ${service} is ${status}; inspect its local logs for diagnostics."
      return 1
    fi
    sleep 10
    elapsed=$((elapsed + 10))
...
run_deploy() {
  ...
  compose config -q
  pull_images_to_log
  note "Creating containers and waiting for model dependencies. First-start disk writes and model initialization can take 15–30 minutes; progress is in the private deployment log."
  if ! compose up -d --pull never --remove-orphans >>"$DEPLOY_LOG" 2>&1; then
    note "Service startup failed. Details are in the private deployment log."
...
refresh-ui  Recreate only the UI container to apply updated routing and assets.
...
case "${1:-}" in
  check) run_check ;;
  deploy) run_deploy ;;
  refresh-ui) compose up -d --pull never --force-recreate --no-deps vss-ui ;;
  status) run_status ;;
  stop) run_stop ;;
  -h|--help|help) usage ;;
  *) usage; exit 2 ;;
